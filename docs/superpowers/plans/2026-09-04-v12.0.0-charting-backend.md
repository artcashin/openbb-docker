...
- Run from `live-grid/`: `pytest`, `ruff check app tests`.
...
def test_cmf_treats_a_flat_bar_as_zero_flow_not_nan():
    ...
    assert out[120] is not None, "one flat bar must not poison the window"
...
def test_vortex_produces_two_columns():
    df = fixture_frame()
    out = compute(df, [resolve("vortex", period=14)])
    for base in ("vi_plus", "vi_minus"):
        name = col("vortex", base, period=14)
        assert out[name][-1] is not None, name


def test_chop_is_bounded_zero_to_one_hundred():
    df = fixture_frame()
...
# live-grid/app/ta/figure.py -- inside build_ta_figure's `for series in pane.series:`
        for series in pane.series:
            ...
            series_x = x if offset == 0 else _dates(
                frame.with_columns(
                    pl.Series("date", shift_times(frame["date"].to_list(), offset))
                )
            )
            trace = {
                "name": series.label, "x": series_x,
                "y": _column(frame, series.column),
                "yaxis": axis, "xaxis": xaxis,
            }
```

Import `shift_times` from `app.ta.panes` at the top of `figure.py` — it already imports `Pane, domains` from there.
...
register(Indicator(
    ...
    render={"supertrend": _line("#98c379"),
            "st_direction": {"type": "line", "color": None, "width": 0}},
...
from app.ta.compute import collect_bases, compute, session_columns
...
def test_session_columns_holds_the_prior_sessions_values_flat():
    frame = _intraday()
    ind = get("pivots_standard")
    out = session_columns(frame, ind, Req("pivots_standard", dict(ind.params)))
    pp = out["PP"]
    # Day 1 has no prior session.
    assert all(v is None for v in pp[:8])
    # Day 2 holds one value across all eight of its bars.
    assert len(set(pp[8:16])) == 1 and pp[8] is not None
...
def test_a_sessioned_and_a_flat_indicator_together_still_share_one_base():
    ...
    sessioned path must not move Base materialisation per-request."""
    reqs = [get_req("pivots_standard"), get_req("atr", period=14),
            get_req("chop", period=14)]
    bases = collect_bases(reqs)
    assert sum(1 for key in bases if key.startswith("tr:")) == 1
    frame = compute(_intraday(days=6), reqs)
    assert "tr:raw:0" not in frame.columns, "base columns are dropped by compute()"


def get_req(name: str, **params) -> Req:
    from app.ta.registry import resolve
...
def session_columns(
    ...
    group_by_dynamic needs a sorted frame -- sorting only before the join_asof
    is too late and silently mis-buckets. shift() is what makes today read
    YESTERDAY's aggregates: without it every level is computed from the
    session it is drawn on, which is not a forecast, it is a lookahead.
    """
    ...
    sessions = (
        ordered.group_by_dynamic("date", every=every, closed="left")
        .agg(**aggregates)
    ...
    joined = ordered.select("date").join_asof(
        sessions.select(["date", *outputs]).sort("date"),
...
def _panes_of(frame: pl.DataFrame, panes: Sequence[Pane]) -> list[dict]:
    spans = domains(list(panes))
    return [
        {"id": pane.id, "height": pane.height, "domain": list(span),
         "guides": list(pane.guides), "series": _series_of(frame, pane)}
...
def series_delta(
    ...
    Offsets are applied to the SLICED frame, which is why a displaced series'
    delta still lands past the last candle: shift_times reads the slice's own
    spacing, and the spacing does not change.
    """
    start = max(0, min(start_row, frame.height))
    tail = frame.slice(start)
    return {"from": start, "candles": _candles(tail),
            "panes": _panes_of(tail, panes)}
```
...
Reuse whatever `client` fixture `test_ta_routes.py` already defines for `/ta_chart_ws`; do not add a second one.
...
"advanced_chart": {
  ...
  "params": [
    ...
    { "paramName": "source", "type": "text", "value": "local", "label": "Price basis",
      "description": "Adjusted or raw close for the indicators. Tick-derived candles are unadjusted either way.",
      "options": [
...
```bash
cd live-grid && python -c "from app.ta.registry import REGISTRY; print(len(REGISTRY))"
```
